...
if (isset($_POST['search-submit'])) {
    ...
    echo("<form method='POST'><input class='' type='submit' name='reload' value='Clear Search Results'></form>");
    }

    
    // Modify the SQL query to search in Name and Description
    $menu_query = mysqli_query($con, "SELECT ItemID, Name, Description, Price, ImagePath, InStock FROM menu WHERE (Name LIKE '%$search_query%' OR Description LIKE '%$search_query%' OR ItemID LIKE '%$search_query%') AND Type='$type'");
    ...
    $menu_query = mysqli_query($con, "SELECT ItemID, Name, Description, Price, ImagePath, InStock FROM menu WHERE Type='$type'");
}
    ...
    while ($row = $menu_query->fetch_assoc()) {
        if ($row["InStock"] == 1) {
            ...
            if($row['ImagePath'] == 'none')
            {
                ...
                {
                    $src = "../images/menuImages/Shake.png";
                }
                else
                {
                    $src = "../images/menuImages/Tea.png";
                }
            }
            else
            {
